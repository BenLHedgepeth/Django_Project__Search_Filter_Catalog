...
def mineral_list(request, query="A"):
    minerals = get_list_or_404(
        Mineral, Q(name__startswith=query) | Q(group=query)
    )
...
def mineral_detail_page(request, name):
    ...
    if not mineral:
        ...
        mineral[0].pop('id')
        mineral = ({key: value
                    for key, value in mineral[0].items() if value != ""})
    search_form = SearchForm(request.GET)
    if search_form.is_valid():
        return HttpResponse(reverse("minerals:search_list"))
    search_form = SearchForm()
...
def show_mineral(request):
    ...
    return HttpResponseRedirect(
        reverse("minerals:detail", kwargs={'name': mineral})
    )
